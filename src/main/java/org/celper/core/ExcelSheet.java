...
//    }
    ...
    private void add(List<ColumnStructure> columnStructures, List<ImportStructure> newList, Cell cell) {
        Optional<ColumnStructure> optionalStructure = columnStructures.stream()
                .filter(structure -> structure.getImportNameOptions().contains(CellUtils.getValue(cell).toString().trim().intern()))
                .findFirst();
